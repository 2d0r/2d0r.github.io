...
export default function ProjectCard({project, imageFit = 'cover'}: {
    ...
    return (
        <motion.div className='md:absolute z-40 md:top-full md:w-[400px] w-full md:left-1/2 mt-2
        ...
        animate={window.innerWidth >= 768 ? {opacity: 1, y: 0, x: '-50%'} : {opacity: 1, height: '12rem', y: 0, x: 0}}
        ...
        key={`${project.title}-card`}>
            <div className='flex flex-col min-w-36 w-1/2 h-full justify-between'>
                <div className='flex flex-col gap-2 overflow-y-clip max-h-[80%]'>
                    <span className='text-xl font-medium text-left'>{project.title}</span>
